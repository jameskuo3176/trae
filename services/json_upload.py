...
负责:
  ...
  2. JSON §6.5 → save_records_to_db 接受的扁平 record 格式
  3. JSON §6.5 → save_violations_to_db 接受的 record 格式
  4. JSON §6.5 → save_notes_to_db 接受的 record 格式
...
def validate_upload_json(data: Any) -> dict:
    """校验 JSON §6.5 顶层结构, 返回规范化后的 dict (含默认值).
...
def json_to_qor_records(
    ...
    """将 JSON §6.5 顶层 records[] 转换为 save_records_to_db 接受的扁平 record 格式.
...
def json_to_violation_records(
    ...
    """将 JSON §6.5 violation_paths[] 转换为 save_violations_to_db 接受的 record 格式"""
    upload = data.get('upload') or {}
...
def json_to_notes_records(
    ...
    """将 JSON §6.5 notes[] 转换为 save_notes_to_db 接受的 record 格式

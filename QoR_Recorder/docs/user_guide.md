...
- 下钻分析违例路径，支持 Bus 合并、跨版本 diff、**多时钟并排对比**（v5.0）
- 一键导出对比结果为 Excel/CSV
- **三级角色模型**（v5.0）：admin / owner / viewer，支持客户/跨团队只读
- **模块级协作**（v5.0）：owner 间可互相授权数据管理权限

## 2. 快速开始

### 2.1 访问系统

1. 打开浏览器，访问 `http://<服务器IP>:5000`（Django 默认端口；生产为 `https://qor.example.com`）
2. 使用管理员分配的账号登录
...
综合策略,compile_ultra,v1.0/variant_a/cpu_core_baseline
综合策略,compile_fast,v1.0/variant_b/cpu_core_baseline
目标频率,500MHz,v1.0/variant_a/cpu_core_baseline
```
...
**模块协作 (v5.0 新增)**: 模块 owner 可在「模块管理」中授权其他 `owner` 用户为协作者. 协作者可上传/管理该模块下数据. viewer 不能成为协作者. 协作者授权仅模块 owner 和 admin 可管理.

**项目软删除 (v4.0+)**:

- admin DELETE 项目 → 软删除 (`status=hidden`), 数据保留, 默认查询不可见
- admin → 已隐藏项目 → 恢复 → 取消软删除
- admin → 已隐藏项目 → 硬删除 (需 `confirm=true`) → 真正删除 `qor_p_<id>.db`
...
### 4.7.1 owner / viewer 角色管理页操作（v5.0 更新）

**owner 角色**（v4.x 的 release 已自动迁移）可访问 `/admin` 页面（仅显示「记录管理」标签），用于管理**自己发布或协作**的记录：

- **数据范围**:
  - 默认 Dashboard "scope=mine"：仅看自己 owner 的模块数据
  - 切换 "scope=all"：看所有已发布数据（与 v4.x release 行为兼容）
- **批量操作**: 选中自己发布 (`released_by=当前用户`) 的记录后可一键撤回
- **不可执行**: 发布他人未发布的记录、撤回他人发布的记录、删除他人记录、上传数据到非协作模块、管理项目/用户

**viewer 角色**（v5.0 新增）登录后：

- 顶部导航**不显示**「管理」按钮
- Dashboard 顶部 banner **无"上传"按钮**
- QoR 记录详情页**无编辑按钮**
...
| 模式             | 切换                | 适用场景                                  |
|------------------|---------------------|-------------------------------------------|
| **时序分析模式** | 关闭"按指标聚合"开关  | 单 metric × 单 clock, 一张图                |
| **违例分析模式** | 开启"按指标聚合"开关  | 多 metric × **多 clock**, 每个 clock 一张子图 |

**多 clock 渲染规则（v5.0 新增）**:

- 时钟多选: 用 `Ctrl/Shift + 点击` 在 `chartTimingClocks` 多选框中选择多个 clock
- 每个 clock 渲染一张独立 ECharts 子图, 容器使用 flex 横向排版
- 子图宽度: 1=100%, 2=50%-6px, 3=33.33%-8px, ≥4=50%-6px (自动换行)
- 子图高度: 1=500px, 2=460px, ≤4=400px, >4=380px
- 颜色编码: WNS=红 (#E04A4A), TNS=深蓝 (#2C3E50), 违例路径数=青 (#7AB7B7)
- 切换 clock 子集时, 容器自动重排, 无残留 canvas

**多时钟数据来源**:

- QoR CSV 中以 `<CLOCK>_period / <CLOCK>_wns / <CLOCK>_tns / <CLOCK>_path` 命名的列自动识别
- 多个 clock 的指标自动存入 `extra_fields`, Dashboard 自动列举
- 旧版 v4.x 仅支持 clock 单选, v5.0 升级为多选 + 并排子图渲染
...
| 文件                                | 存储内容                                                                                |
|-------------------------------------|-----------------------------------------------------------------------------------------|
| `qor_recorder.db`（主库）            | 用户、项目元数据、API Key、项目成员、仪表板配置主键、ReviewSnapshot 等**系统级**数据       |
| `qor_p_<id>.db`（项目库，**每个项目一个**） | 该项目的模块、QoR 记录、违例路径、Run 备注、Tile/Group/Subsystem Review、告警规则等**业务**数据 |

> v5.0 增加了 `modules.owner_id` 与 `modules.collaborators` 字段, 用于 owner 间模块协作.
...
| 角色     | 数据可见性                                                    | 写操作                |
|----------|----------------------------------------------------------------|------------------------|
| `admin`  | 所有数据 (含 hidden/已删除)                                    | 所有项目所有操作        |
| `owner`  | 自己 owner + 协作者模块的全部数据 + 任何已发布数据              | 自己 + 协作模块的 CRUD |
| `viewer` | 仅 `is_released=True` 的数据                                   | 无                      |

> v4.x 的 `user` 角色在升级时自动迁移为 `owner`. v4.x 的 `release` 角色也迁移为 `owner`, 历史 `release` 用户的发布/撤回权限保留.

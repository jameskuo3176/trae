...
from django_app.core.db_routing import (
    _get_legacy_project_db_alias, get_legacy_project_engine, project_db_path,
)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        totals = {'qor_records': 0, 'raw_reports': 0, 'violation_paths': 0, 'run_notes': 0}
        failures = []
        for project in projects:
            try:
                ...
                self.stderr.write(
                    self.style.ERROR(
                        f'project={project.id} source={project_db_path(project.id)} error={exc}'
                    )
                )
                continue
            ...
            counts = {
                ...
                'raw_reports': records.exclude(raw_dc_report__isnull=True).exclude(
                    raw_dc_report=''
                ).count(),
        ...
        summary = f'{mode} source_totals={totals} failures={len(failures)}'
        if failures:
            raise CommandError(f'{summary}; errors={failures}')

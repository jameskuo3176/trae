...
def _build_extra_fields(dc: dict, default_path: Optional[str]) -> dict:
    """构造 record.extra (审计用).

    规则: 除时序标签外, 所有非时序字段不得包含 'scenarios' 字样。
    - scenarios 审计数据仅存在于 extra.scenarios (允许的例外)
    - timing_final.scenarios 属于时序数据, 允许保留
    """
    extra = {}
    if dc.get('stage'):
    ...
    if isinstance(md, dict):
        for k in ('report', 'design', 'version', 'date'):
            if md.get(k):
...
def convert_dc_to_qor_record(dc: dict, *,
    ...
    # extra 字段 (审计 + 聚合)
    extra = _build_extra_fields(dc, default_path)
    if scenarios_audit:
...
def validate_dc_report(data: Any) -> dict:
    ...
    层级关系:
      - Run 级 (顶层): area, misc, top_module, run, stage, errors
        → 物理实现指标: 面积/单元数/MBB/时钟门控/利用率/拥塞 (每个 run 仅一个汇总值)
      - Scenario 级 (timing.default.scenarios / timing.final.scenarios):
        → 时序分析数据: WNS/TNS/NVP/Clk_Period/LoL (每个 scenario 一个值)
      - 物理指标 不得 嵌套在 scenarios 内部, 二者是独立的层级

    数据规则:
      1. 除时序标签外, 所有非时序字段不得包含 'scenarios' 字样
      2. 每个 run 仅对应一组数值参数 (一对一约束)
      3. scenarios 字段是唯一例外, 允许一个 run 关联多个场景条目
    """
    ...
    # 规则1: 扫描非时序字段, 禁止包含 'scenarios' 字样
    _validate_no_scenarios_in_non_timing_fields(data)

    # 规则2: 验证 run 级数值的一对一约束
    _validate_run_level_uniqueness(data)

    return data


def _validate_run_level_metrics(data: dict) -> None:
    """验证 area, misc 等物理指标在顶层 (run 级), 而非嵌套在 scenarios 中.

    检测: 若 scenarios 内的 path_group 包含 area/misc 等字段, 则报告错误.
    """
    # 检查 area 是否在顶层
    area = data.get('area')
    if area is not None and not isinstance(area, dict):
        raise DCReportError('$.area', 'area 必须是对象 (run 级物理指标)')

    # 检查 misc 是否在顶层
    misc = data.get('misc')
    if misc is not None and not isinstance(misc, dict):
        raise DCReportError('$.misc', 'misc 必须是对象 (run 级物理指标)')

    # 检查 timing.scenarios 内部是否误放了物理指标
    timing = data.get('timing') or {}
    for timing_type in ('default', 'final'):
        ...
        for sname, sval in scens.items():
            if not isinstance(sval, dict):
                continue
            # path_groups 内的每个 clock 只应包含时序字段
            pgs = sval.get('path_groups') or {}
            for gname, gval in pgs.items():
                if not isinstance(gval, dict):
                    continue
                invalid_keys = [k for k in gval if k.lower() in ('area', 'misc', 'cells', 'mbb_ratio',
                    'clock_gating', 'utilization', 'congestion', 'fgcg', 'vt_ratio')]
                if invalid_keys:
                    raise DCReportError(
                        f'$.timing.{timing_type}.scenarios.{sname}.path_groups.{gname}',
                        f'物理指标 {invalid_keys} 不应嵌套在 scenario 内, '
                        f'请放置在顶层 area / misc 中'
                    )


def _validate_no_scenarios_in_non_timing_fields(data: dict, prefix: str = '$') -> None:
    """规则1: 扫描非时序字段, 禁止包含 'scenarios' 字样.
    ...
    def _is_timing_path(path: str) -> bool:
        """判断路径是否属于时序 scenarios 标签 (允许的例外)."""
        for tp in _TIMING_PREFIXES:
    ...
    def _scan(obj, path: str):
        if isinstance(obj, dict):
            for key, value in obj.items():
                ...
                # 检查键名是否包含 scenario (非时序路径)
                if _SCENARIO_LOWER in key.lower():
                    if not _is_timing_path(current_path):
                        raise DCReportError(
                            current_path,
                            f'非时序字段 "{key}" 不得包含 "scenarios" 字样. '
                            f'仅 timing.*.scenarios 路径下允许使用该词.'
                        )
...
def _validate_run_level_uniqueness(data: dict) -> None:
    """规则2: 验证 run 级数值参数的一对一约束.

    每个 run 仅对应一组数值参数:
      - area 字段必须是标量对象 (非数组)
      - misc 字段必须是标量对象 (非数组)
      - cells/cell_count 字段必须是标量对象 (非数组)
      - 以上字段不得包含多个值 (数组) 或多组值 (嵌套 dict of dict)

    唯一的例外: timing.*.scenarios 允许一个 run 关联多个场景条目.
    """
    _RUN_LEVEL_FIELDS = {
        'area': '$.area',
        'misc': '$.misc',
    ...
    for field, json_path in _RUN_LEVEL_FIELDS.items():
        ...
        if isinstance(value, list):
            raise DCReportError(
                json_path,
                f'"{field}" 字段必须是单个对象 (一个 run 仅对应一组数值), '
                f'不允许数组形式. 多个值应分别属于不同 run.'
            )
        ...
        for sub_key, sub_value in value.items():
            ...
            if isinstance(sub_value, list):
                raise DCReportError(
                    sub_path,
                    f'"{sub_path}" 字段必须是单个对象, 不允许数组形式. '
                    f'一个 run 仅对应一组数值.'
                )

    # 检查 area.tile 内的子字段是否为数组
    area = data.get('area') or {}
    if isinstance(area, dict):
        tile = area.get('tile') or {}
        if isinstance(tile, dict):
            for tile_key in ('area', 'cell_count', 'flop_count'):
                tile_val = tile.get(tile_key)
                if isinstance(tile_val, list):
                    raise DCReportError(
                        f'$.area.tile.{tile_key}',
                        f'"{tile_key}" 字段必须是单个对象, 不允许数组形式. '
                        f'一个 run 仅对应一组数值.'
                    )
